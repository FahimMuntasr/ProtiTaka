  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
      <section className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl shadow-black/30">
        <p className="text-sm uppercase tracking-[0.35em] text-emerald-400">Login</p>
        <h1 className="mt-3 text-3xl font-semibold text-white">Welcome back</h1>
        <p className="mt-3 text-slate-300">Sign in to view your transactions and daily overview.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <button disabled={loading} className="w-full rounded-full bg-emerald-400 px-5 py-3 text-sm font-semibold text-slate-950 hover:bg-emerald-300 disabled:opacity-70">{loading ? 'Signing in...' : 'Sign in'}</button>
        </form>

        {message ? <p className="mt-4 text-sm text-rose-300">{message}</p> : null}

        <p className="mt-6 text-sm text-slate-300">No account yet? <Link to="/signup" className="text-emerald-300">Create one</Link></p>
      </section>
    </main>
  )
}
